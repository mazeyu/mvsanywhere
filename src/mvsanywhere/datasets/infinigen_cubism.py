...
class InfinigenCubismDataset(GenericMVSDataset):
    """
    InfinigenCubism Dataset class.
    ...
    def update_image_size(self, scan_id, frame_id):
        filepath = self.get_color_filepath(scan_id, frame_id)
        img = pil.open(filepath)
        image_width0, image_height0 = img.size
        multiple = 16
        image_width = int(image_width0*5/6 // multiple) * multiple
        image_height = int(image_height0*5/6 // multiple) * multiple

        # Random resize crop
        random_resize_crop = transforms.RandomResizedCrop((image_height, image_width), scale=(0.75, 1.0), ratio=(image_width/image_height, image_width/image_height))

        return image_width0, image_height0, image_width, image_height, random_resize_crop
    ...
    def get_high_res_color_filepath(self, scan_id, frame_id):
        ...
        return os.path.join(self.scenes_path, scan_id, "images", f"{frame_id}.png")
    ...
    def get_full_res_depth_filepath(self, scan_id, frame_id):
        ...
        return os.path.join(self.scenes_path, scan_id, "depths", f"{frame_id}.npy")
    ...
    def load_full_res_depth_and_mask(self, scan_id, frame_id):
        ...
        # Get the float valid mask
        full_res_mask_b = (full_res_depth > 0) & (full_res_depth < 1e3)
